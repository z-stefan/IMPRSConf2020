...
* you have created an R project with version control from [here](https://github.com/eirini-zormpa/IMPRS_Conf2020).
...
# Set up
Most of what we'll cover today does not actually involve much code--we'll mostly cover how to set up your projects  with reproducibility in mind.
...
We'll be using ```rrtools``` to create some of our documentation.
Packages like ```rrtools``` are great for learning reproducibility because:

1. they make it easy to not forget any steps
2. they come with templates and clear instructions on what to include to write good documentation.

You can read more about the ```rrtools``` package [here](https://github.com/benmarwick/rrtools).
To install the ```rrrtools``` package you just need to run the command:
...
As you are the copyright holder, anyone who wants to use your work should technically ask for your permission to be do do.
...
But as we all know, just because you *could* do something doesn't mean you *should*.
...
The ```rrtools``` package gives a super easy way of attaching an MIT license to our work, so we'll just go ahead and use that.
To do that, you just need to run the command ```usethis::use_mit_license(name = "My Name")```.
Make sure to run commands like this, relating to the structure of your project, in your *console*.
This is not an action you want to be repeating, so don't include it in your script.

So go to your console, copy the command above, and replace "My Name" with, well, your name.
Check your project, you should see a file called "LICENSE".
...
Go ahead and click on the box to add the README file and commit it with an informative message.
